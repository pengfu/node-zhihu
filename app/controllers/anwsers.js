...
class QuestionCtl {
  ...
  async findById(ctx) {
    ...
    const selectFields = fields
      ...
      .map((f) => ' +' + f)
      .join('')
    const topic = await Anwser.findById(ctx.params.id)
      .select(selectFields)
      .populate('answerer')
    ctx.body = topic
  }
  ...
  async update(ctx) {
    ...
    const Anwser = await Anwser.findByIdAndUpdate(
      ctx.params.id,
      ctx.request.body
    )
    ctx.body = Anwser
  ...
  async checkAnwserExist(ctx, next) {
    ...
    if (!anwser) {
      ctx.throw(404, 'Anwser is not exist!')
    }
