...
class UserController {
  ...
  async find(ctx) {
    ...
    ctx.body = await Users.find({ name: new RegExp(ctx.query.q) })
      .limit(perPage)
      .skip((page - 1) * perPage)
  }
  async findById(ctx) {
    ...
    const populateStr = fields
      ...
      .map((f) => {
        ...
        if (f === 'educations') return 'educations.schol educations.major'
        return f
      })
      .join(' ')
  ...
  async delete(ctx) {
    ...
    if (!user) {
      ctx.throw(404, 'user not found (delete)')
    }
    ctx.status = 204
  }
  async login(ctx) {
  ...
  async listLikingAnwsers(ctx) {
    const user = await Users.findById(ctx.params.id)
      .select('+likingAnwsers')
      .populate('likingAnwsers')
    if (!user) {
      ctx.throw(404, '用户不存在')
    }
  ...
  async undislikeAnwser(ctx) {
    ...
    const index = me.dislikingAnwsers
      .map((x) => x.toString())
      .indexOf(ctx.params.id)
    if (index !== -1) {
  ...
  async listdisLikingAnwsers(ctx) {
    const user = await Users.findById(ctx.params.id)
      .select('+dislikingAnwsers')
      .populate('dislikingAnwsers')
    if (!user) {
      ctx.throw(404, '用户不存在')
    }
  ...
  async collectAnwser(ctx, next) {
    ...
    if (
      !me.collectingAnwsers.map((x) => x.toString()).includes(ctx.params.id)
    ) {
  ...
  async unCollectAnwser(ctx) {
    ...
    const index = me.collectingAnwsers
      .map((x) => x.toString())
      .indexOf(ctx.params.id)
    if (index !== -1) {
  ...
  async listCollectAnwsers(ctx) {
    const user = await Users.findById(ctx.params.id)
      .select('+collectingAnwsers')
      .populate('collectingAnwsers')
    if (!user) {
      ctx.throw(404, '用户不存在')
    }
